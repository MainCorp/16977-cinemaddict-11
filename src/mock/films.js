...
const generateFilm = () => {
  ...
  return {
    ...
    "film_info": {
      ...
      "writers": createRandomCollection(writersList, 1, writersList.length, `,`).split(`,`),
      "actors": createRandomCollection(actorsList, 1, actorsList.length, `,`).split(`,`),
      "release": {
        "date": generateDate(),
        "release_country": countryList[Math.floor(Math.random() * countryList.length)],
      },
      "runtime": runtime,
      "genre": createRandomCollection(genreList, 1, genreList.length, `,`).split(`,`),
